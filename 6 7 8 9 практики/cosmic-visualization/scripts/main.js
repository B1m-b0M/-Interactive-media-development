...
function selectEvent(eventType) {
    ...
    // Гейміфікація: додати подію
    if (window.Gamification) {
        Gamification.addEvent(eventType);
    }
...
function animateCosmicObject(object, timeScale, mass, intensity) {
    ...
    switch (currentEvent) {
        case 'supernova': {
            ...
            shock.className = 'supernova-shockwave';
            shock.style.opacity = 0.5 + 0.5 * Math.abs(Math.sin(time * 2));
            shock.style.transform = `scale(${1 + Math.abs(Math.sin(time * 2)) * 1.5})`;
            object.appendChild(shock);
            break;
        }
        case 'blackhole': {
            ...
            shadow.className = 'blackhole-shadow';
            object.appendChild(shadow);
            // Акреційний диск
            let disk = document.createElement('div');
            disk.className = 'accretion-disk';
            disk.style.transform = `rotate(${time * 30}deg)`;
            object.appendChild(disk);
            // Гравітаційне лінзування (кільце)
            let ring = document.createElement('div');
            ring.className = 'blackhole-ring';
            ring.style.boxShadow = `0 0 32px 8px #ffd60088, 0 0 80px 10px #64b5f644`;
            object.appendChild(ring);
            // Анімація поглинання (ефект "затягування")
            object.style.animation = 'blackhole-absorb 2.5s infinite alternate';
            break;
        }
        case 'neutron': {
            ...
            // Додаємо "маячок" пульсара
            let beam = document.createElement('div');
            beam.className = 'neutron-beam';
            beam.style.transform = `rotate(${time * 180}deg)`;
            object.appendChild(beam);
            break;
        }
    }
}
